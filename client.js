// workspace-files-explorer · Client 半部分
// 使用方式：把本文件内容整体作为 cordis_define 的 code.client 使用。
// ---------------------------------------------------------------------------
// 职责：
//   - 会话头部动作区注册「工作区文件」按钮（切换面板显隐）
//   - shell.overlay 浮层渲染文件浏览器面板：左侧文件树 + 右侧代码/Markdown 预览
//   - 工作区根：useSessions((s) => s) 读取当前会话 cwd，随每次 RPC 传给 Host；切换会话自动重载
//   - 预览安全：所有内容先 HTML 转义再渲染；Markdown 链接仅允许 http/https 且 rel=noopener
//   - 内置轻量语法高亮器与 Markdown 渲染器，无任何第三方依赖
// 说明：仅使用 React（createElement/useState/useEffect）、host.call、
//       styles.insert 与 slots 服务；所有副作用随插件 Fiber 清理。
// ---------------------------------------------------------------------------
const CSS = `
...
function TreeItem(props) {
  ...
  function load() {
    ...
    host.call('wsfiles.list', { cwd: props.cwd, path: entry.path }).then(function (res) {
...
function PanelInner(props) {
  ...
  React.useEffect(function () {
    ...
    setSelected(null)
    setPreview(null)
    host.call('wsfiles.root', { cwd }).then(function (res) {
      if (!alive) return
  ...
  function openPreview(path, name) {
    ...
    host.call('wsfiles.read', { cwd, path }).then(function (res) {
