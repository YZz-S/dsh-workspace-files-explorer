...
![文件预览效果图_1](./images/%E6%96%87%E4%BB%B6%E9%A2%84%E8%A7%88%E6%95%88%E6%9E%9C%E5%9B%BE_1.png)

![文件预览效果图_2](./images/%E6%96%87%E4%BB%B6%E9%A2%84%E8%A7%88%E6%95%88%E6%9E%9C%E5%9B%BE_2.png)
...
| 文件 | 说明 |
| --- | --- |
| `host.js` | Host 半部分：`wsfiles.root` / `wsfiles.list` / `wsfiles.read` 三个包私有 RPC，负责路径解析、工作区边界校验与文本读取 |
| `client.js` | Client 半部分：渲染文件树与预览面板、内置语法高亮器与 Markdown 渲染器，通过 `host.call` 调用 Host |
| `README.md` | 本说明 |
| `LICENSE` | MIT 许可证 |
...
把 Host / Client 逻辑封装为 npm 包并挂载到 profile（参见 DeepSeek Harness 插件开发文档）。`host.js` / `client.js` 均为可直接复用的函数体。

## 工作原理

- **工作区根**：Client 端通过标准 hook `useSessions((s) => s)` 读取当前会话的 `cwd`，随每次 RPC 传给 Host；无会话时回退 `sandboxPolicy.workspaceRoot`
...
- 仅读取当前会话工作区内的文件与目录列表，**无网络请求、无文件写入、无遥测、无持久化存储**
...
- [x] 无网络请求、无遥测、无第三方数据收集
- [x] MIT 许可证齐全，README 完整
- [x] 代码仅使用 dsh 动态插件公开接口（Services / harness.handle / slots / styles / React），副作用全部挂在插件 Fiber 上，停止即清理
